...
#   scripts/agent-context.sh <agent-name>    # a subagent of that session, by the name it was spawned with
#   scripts/agent-context.sh --session <id> [agent-name]
#
# Prints one line:  live=<tokens> peak=<tokens> turns=<n> compacted=<yes|no> file=<path>
# "live" is input + cache_read + cache_creation of the latest assistant turn with usage, which is what the
...
if [[ -n "$agent" ]]; then
  file="$(ls -t "$project_dir/$session/subagents/"agent-*"$agent"-*.jsonl 2>/dev/null | head -1)"
...
jq -rs --arg file "$file" '
  ...
  | "live=\($live) peak=\($ctx | max // 0) turns=\($ctx | length) compacted=\(if any(.[]; .isCompactSummary == true or .type == "compact_boundary") then "yes" else "no" end) file=\($file)"
' "$file"
